...
│           ├── config/          # チェーン・Web3 設定
│           ├── controllers/     # ビュー別ビジネスロジック（useXxxController）
│           ├── exceptions/      # カスタム例外定義
│           ├── hooks/           # 汎用カスタムフック
│           ├── i18n/            # 多言語対応
│           │   └── locales/     # ja / en / ko / zh
...
│           ├── styles/          # グローバル CSS
│           ├── utils/           # ユーティリティ・バリデータ
│           │   └── validators/  # アドレス・金額バリデーション
│           └── views/           # 画面コンポーネント
├── functions/                   # Firebase Cloud Functions
│   └── src/
│       ├── index.ts             # エントリポイント・全 API の re-export
│       ├── common/              # 共通ユーティリティ
│       │   ├── auth.ts          # 認証ヘルパー
│       │   ├── cors.ts          # CORS 設定
│       │   ├── db.ts            # TiDB 接続・クエリ
│       │   ├── notificationSenders.ts
│       │   └── planLimits.ts    # プランごとの制限値
│       └── api/                 # 機能別 API（各フォルダに handler ファイル）
│           ├── auth/            # 認証系（Google / Apple / LINE / Wallet）
│           ├── billing/         # Stripe 課金
│           ├── contact/         # コンタクト管理
│           ├── information/     # お知らせ
│           ├── label/           # ラベル管理
...
│           ├── rpc/             # RPC ヘルスチェック
│           ├── system/          # システム管理（クリーンアップ等）
│           ├── transaction/     # トランザクション同期・取得
│           ├── version/         # バージョン管理
│           └── wallet/          # ウォレット管理
...
`functions/src/api/version/` 配下を参考にする。パターン:
1. `functions/src/api/<機能>/` フォルダを作成
2. 各ファイルに handler 関数を実装し `export const functionNameDev/Prd` を追加
...
- [ ] エラーを握りつぶしていないか？
- [ ] WEB標準の機能か。ELCTORN/TAURI専用の機能使っていないか？
- [ ] WEBでは動くがELCTORN/TAURIで動かない設計ににあっていないか？
- [ ] フォールバックで `null` / 空配列 / デフォルト値を黙って返していないか？
- [ ] 仮実装・デバッグコードにはユーザーへの告知と `TODO(仮):` コメントがあるか？
- [ ] 型定義はあるか？（`any` を使っていないか？）
- [ ] 開発環境（DEV)の調整が本番環境（PRD)に影響しないようになっているか？
- [ ] 過度な抽象化・早すぎる最適化をしていないか？
- [ ] Functions を追加するときは firebase.json の rewrite も一緒に更新する
