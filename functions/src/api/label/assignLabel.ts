...
async function handleAssignLabel(
  ...
  try {
    // ラベルがユーザー所有かチェック
    const [labelRows] = await conn.execute<RowDataPacket[]>(
      `SELECT id FROM transaction_labels_t WHERE id = ? AND user_id = ? AND delete_flg = 0`,
      [labelId, userId],
    );
    ...
    // user_transactions_t でユーザー所有かチェック
    ...
    functions.logger.error('handleAssignLabel error', {
