...
const fullNameArb = fc.stringMatching(/^[a-z ]{2,20}$/);
 ...
 */
async function sendKycNotificationsCore(recipients, data, sendNotification) {
    ...
    for (const recipient of recipients) {
        ...
        if (kycRequests) {
            const hasCompleted = Object.values(kycRequests).some((entry) => entry.recipientAddress === normalizedAddress && entry.status === 'completed');
            if (hasCompleted) {
                skipped++;
 ...
 */
async function submitKycCore(input, data) {
    const kycRequestPath = `${DB_ROOT}/kycRequests/${input.token}`;
    const kycRequest = getNestedValue(data, kycRequestPath);
    if (!kycRequest) {
    ...
    if (input.recoveredAddress.toLowerCase() !== kycRequest.recipientAddress.toLowerCase()) {
...
describe('sendKycNotifications / submitKyc Property Tests', () => {
    ...
    describe('Property 5: 通知送信の冪等性', () => {
        it('KYC完了済みアドレスには通知を送信しない（skippedにカウント）', async () => {
            await fc.assert(fc.asyncProperty(fc.array(ethAddressArb, { minLength: 1, maxLength: 5 }), fc.array(fc.boolean(), { minLength: 5, maxLength: 5 }), emailArb, sessionNameArb, async (addresses, booleans, email, sessionName) => {
                // Deduplicate addresses
                const uniqueAddresses = [...new Set(addresses.map((a) => a.toLowerCase()))].map((a) => `0x${a.slice(2)}`);
                ...
                const recipients = uniqueAddresses.map((addr) => ({
                    address: addr,
                ...
                uniqueAddresses.forEach((addr, i) => {
                    if (completedFlags[i]) {
                        const detail = result.details.find((d) => d.address === addr && d.status === 'skipped');
    ...
    describe('Property 3: KYC完了→承認済みアドレス追加の原子性', () => {
        it('KYC完了時に必ず承認済みアドレスにも追加される', async () => {
            await fc.assert(fc.asyncProperty(ethAddressArb, tokenArb, fullNameArb, emailArb, sessionNameArb, async (address, token, fullName, email, sessionName) => {
                const data = {};
                const normalizedAddress = address.toLowerCase();
                ...
                const approvedAddress = getNestedValue(data, `${DB_ROOT}/approvedAddresses/${normalizedAddress}`);
        ...
        it('KYC失敗時には承認済みアドレスが追加されない', async () => {
            await fc.assert(fc.asyncProperty(ethAddressArb, ethAddressArb, tokenArb, fullNameArb, emailArb, sessionNameArb, async (address, wrongAddress, token, fullName, email, sessionName) => {
    ...
    describe('Property 4: 重複KYC防止', () => {
        ...
        it('KYC完了済みアドレスへの通知送信は冪等にスキップされる', async () => {
            await fc.assert(fc.asyncProperty(ethAddressArb, tokenArb, fullNameArb, emailArb, sessionNameArb, fc.integer({ min: 2, max: 5 }), async (address, token, fullName, email, sessionName, repeatCount) => {
                const data = {};
                const normalizedAddress = address.toLowerCase();
                ...
                const recipients = Array.from({ length: repeatCount }, () => ({
                    address,
                    notificationType: 'email',
                    notificationId: email,
                    sessionName,
...
//# sourceMappingURL=sendKycNotifications.property.test.js.map