...
export default function App() {
    ...
    async function loadBillingPlans() {
        try {
            const token = await user.getIdToken();
            const data = await apiFetch(api('getBillingPlans'), token);
    ...
    async function handleSaveBillingPlan(e) {
        e.preventDefault();
        if (!editingPlan)
            return;
        setSaving(true);
        ...
        try {
            const token = await user.getIdToken();
            await apiFetch(api('upsertBillingPlan'), token, editingPlan);
            setMsg('保存しました');
            setEditingPlan(null);
    ...
    async function handleDeleteBillingPlan(id, name) {
        if (!confirm(`「${name}」を削除しますか？`))
            return;
        const token = await user.getIdToken();
        await apiFetch(api('deleteBillingPlan'), token, { id });
        setMsg('削除しました');
        await loadBillingPlans();
    }
    ...
    async function handleAddExcludeUser() {
        ...
        try {
            const token = await user.getIdToken();
            await apiFetch(api('addExcludeUser'), token, { user_id: newUserId.trim() });
            setMsg('追加しました');
            setNewUserId('');
