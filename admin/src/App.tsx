...
type Tab = 'versions' | 'maintenance' | 'excludeUsers' | 'information' | 'billingPlans';
...
export default function App() {
  ...
  useEffect(() => { if (user) { loadVersions(); loadMaintenances(); loadExcludeUsers(); loadInformations(); loadBillingPlans(); } }, [user]);
  ...
  async function handleSaveBillingPlan(e: React.FormEvent) {
    e.preventDefault(); if (!editingPlan) return;
    setSaving(true); setMsg('');
    try {
      const token = await user!.getIdToken();
      await apiFetch(api('upsertBillingPlan'), token, editingPlan);
      setMsg('保存しました'); setEditingPlan(null); await loadBillingPlans();
    } catch (e: any) { setMsg('エラー: ' + e.message); }
    finally { setSaving(false); }
  }

  async function handleDeleteBillingPlan(id: number, name: string) {
    if (!confirm(`「${name}」を削除しますか？`)) return;
    const token = await user!.getIdToken();
    await apiFetch(api('deleteBillingPlan'), token, { id });
    setMsg('削除しました'); await loadBillingPlans();
  }
  ...
  async function handleAddExcludeUser() {
    ...
    try {
      ...
      setMsg('追加しました'); setNewUserId(''); await loadExcludeUsers();
    } catch (e: any) { setMsg('エラー: ' + e.message); }
  }
  ...
  return (
    <div style={s.page}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e5e7eb', display: 'flex', gap: 0 }}>
        {([['versions', 'バージョン管理'], ['maintenance', 'メンテナンス'], ['excludeUsers', '除外ユーザー'], ['information', 'お知らせ'], ['billingPlans', 'プラン管理']] as [Tab, string][]).map(([key, label]) => (
      ...
      <div style={s.container}>
        ...
        {tab === 'excludeUsers' && (
          <div style={s.card}>
            ...
            <p style={{ fontSize: 13, color: '#6b7280', marginBottom: 16 }}>
              メンテナンスステータスが「2」の場合、このリストのユーザーIDはメンテナンス画面をスキップできます。
            </p>
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              <input style={{ ...s.input, flex: 1 }} placeholder="ユーザーID" value={newUserId} onChange={e => setNewUserId(e.target.value)} />
              <button style={s.btnPrimary} onClick={handleAddExcludeUser}>追加</button>
            </div>
            <table style={s.table}>
              <thead><tr style={{ background: '#f0f4ff' }}>
                {['ユーザーID', '登録日時', '操作'].map(h => <th key={h} style={s.th}>{h}</th>)}
              </tr></thead>
              <tbody>
                {excludeUsers.length === 0 && <tr><td colSpan={3} style={{ textAlign: 'center', padding: 24, color: '#999' }}>データなし</td></tr>}
                {excludeUsers.map(u => (
                  <tr key={u.id} style={{ borderBottom: '1px solid #eee' }}>
                    <td style={{ ...s.td, fontFamily: 'monospace', fontSize: 13 }}>{u.user_id}</td>
        ...
        {tab === 'billingPlans' && (
          <div style={s.card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h2 style={{ fontSize: 16, fontWeight: 600 }}>プラン管理（billing_plan_m）</h2>
              <button style={s.btnPrimary} onClick={() => setEditingPlan({ plan_key: '', billing_type: 'one_time', display_name_ja: '', display_name_en: '', unit_amount: 0, currency: 'jpy', description_ja: null, is_active: 1, sort_order: 0 })}>+ 追加</button>
            </div>
            <div style={{ marginBottom: 12, padding: 12, background: '#f8fafc', borderRadius: 8, fontSize: 13 }}>
              <b>plan_key の予約値：</b>　<code>light</code>（ライト ¥300）　／　<code>heavy</code>（ヘビー ¥1,900）　／　<code>upgrade_light_to_heavy</code>（L→Hアップグレード ¥1,600）
            </div>
            <table style={s.table}>
              <thead><tr style={{ background: '#f0f4ff' }}>
                {['plan_key', '表示名（JA）', '金額', '通貨', '有効', 'sort', '更新日時', '操作'].map(h => <th key={h} style={s.th}>{h}</th>)}
              </tr></thead>
              <tbody>
                {billingPlans.length === 0 && <tr><td colSpan={8} style={{ textAlign: 'center', padding: 24, color: '#999' }}>データなし</td></tr>}
                {billingPlans.map(plan => (
                  <tr key={plan.id} style={{ borderBottom: '1px solid #eee', opacity: plan.is_active ? 1 : 0.5 }}>
                    <td style={{ ...s.td, fontFamily: 'monospace', fontSize: 13 }}><span style={s.badge}>{plan.plan_key}</span></td>
                    <td style={s.td}>{plan.display_name_ja}</td>
                    <td style={{ ...s.td, fontWeight: 700 }}>¥{plan.unit_amount.toLocaleString()}</td>
                    <td style={{ ...s.td, fontSize: 13, color: '#666' }}>{plan.currency}</td>
                    <td style={{ ...s.td, textAlign: 'center' }}>
                      <span style={{ color: plan.is_active ? '#16a34a' : '#dc2626', fontWeight: 700, fontSize: 13 }}>{plan.is_active ? '有効' : '無効'}</span>
                    </td>
                    <td style={{ ...s.td, textAlign: 'center', color: '#666' }}>{plan.sort_order}</td>
                    <td style={{ ...s.td, fontSize: 12, color: '#666' }}>{plan.updated_at ? new Date(plan.updated_at).toLocaleString('ja-JP') : '-'}</td>
                    <td style={s.td}>
                      <button style={{ ...s.btnSecondary, marginRight: 6 }} onClick={() => setEditingPlan({ ...plan })}>編集</button>
                      <button style={s.btnDanger} onClick={() => handleDeleteBillingPlan(plan.id, plan.display_name_ja)}>削除</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {editingPlan && (
        <div style={s.overlay}>
          <div style={s.modal}>
            <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 20 }}>{editingPlan.id ? 'プラン編集' : 'プラン追加'}</h2>
            <form onSubmit={handleSaveBillingPlan} style={s.form}>
              <label style={s.label}>plan_key（例: light / heavy / upgrade_light_to_heavy）</label>
              <input style={s.input} value={editingPlan.plan_key || ''} onChange={e => setEditingPlan({ ...editingPlan, plan_key: e.target.value })} required placeholder="light" />
              <label style={s.label}>表示名 🇯🇵 日本語</label>
              <input style={s.input} value={editingPlan.display_name_ja || ''} onChange={e => setEditingPlan({ ...editingPlan, display_name_ja: e.target.value })} required placeholder="ライトプラン" />
              <label style={s.label}>表示名 🇺🇸 英語</label>
              <input style={s.input} value={editingPlan.display_name_en || ''} onChange={e => setEditingPlan({ ...editingPlan, display_name_en: e.target.value })} placeholder="Light Plan" />
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={s.label}>金額（整数）</label>
                  <input style={s.input} type="number" min={0} value={editingPlan.unit_amount ?? 0} onChange={e => setEditingPlan({ ...editingPlan, unit_amount: Number(e.target.value) })} required />
                </div>
                <div>
                  <label style={s.label}>通貨</label>
                  <select style={s.input} value={editingPlan.currency || 'jpy'} onChange={e => setEditingPlan({ ...editingPlan, currency: e.target.value })}>
              ...
              <textarea style={{ ...s.input, height: 60, resize: 'vertical' }} value={editingPlan.description_ja || ''} onChange={e => setEditingPlan({ ...editingPlan, description_ja: e.target.value || null })} />
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={s.label}>有効フラグ</label>
                  <select style={s.input} value={editingPlan.is_active ?? 1} onChange={e => setEditingPlan({ ...editingPlan, is_active: Number(e.target.value) })}>
                    <option value={1}>有効</option>
                    <option value={0}>無効</option>
                  </select>
                </div>
                <div>
                  <label style={s.label}>表示順（sort_order）</label>
                  <input style={s.input} type="number" value={editingPlan.sort_order ?? 0} onChange={e => setEditingPlan({ ...editingPlan, sort_order: Number(e.target.value) })} />
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                <button style={s.btnPrimary} type="submit" disabled={saving}>{saving ? '保存中...' : '保存'}</button>
                <button style={s.btnSecondary} type="button" onClick={() => setEditingPlan(null)}>キャンセル</button>
              </div>
            </form>
          </div>
        </div>
