...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  const {
    loading, error,
    handleWalletConnect, handleCancelWallet, handleGoogleLogin, handleGuestLogin, isButtonLoading,
        ...
        <div className="space-y-3">
          ...
          {isButtonLoading('wallet') ? (
            <button
              ...
              className="w-full bg-blue-600 text-white py-3 px-6 rounded-lg font-medium flex items-center justify-center gap-2"
            >
              <Spinner />{t('login.signing')}
              <span className="text-blue-200 text-sm">({t('common.cancel')})</span>
            </button>
          ) : (
            <button
