...
const Header: React.FC<HeaderProps> = ({
  ...
  const loginDisplay =
    loginType === 'wallet' && loginIdentifier
      ? formatAddress(loginIdentifier)
        ...
        <div className="flex items-center gap-2 md:gap-3 shrink-0">
          ...
          {isConnected && address ? (
            <button
              ...
              className="text-xs text-blue-600 border border-blue-200 px-2 py-1 rounded-lg hover:bg-blue-50 transition-colors font-mono"
              title={t('header.switchWallet')}
            >
              {formatAddress(address)}
            </button>
          ) : (
            <button onClick={onConnect} className="btn-primary text-xs px-2 md:px-4">
              <span className="hidden sm:inline">{t('header.connectWallet')}</span>
              <span className="sm:hidden">{t('header.connect')}</span>
            </button>
          )}
