...
トランザクションデータは「オンチェーン事実」と「ユーザー固有の状態」を分離して管理する。

```
transactions_t          … オンチェーンの事実（ユーザー非依存、1tx=1レコード）
  ↑
user_transactions_t     … ユーザー×txの紐付け（direction, state, watched_address）
  ↑
transaction_label_assignments_t … ユーザーが付けたラベル
```

- 同一txを複数ユーザーが監視しても `transactions_t` は1行のみ
- `INSERT ... ON DUPLICATE KEY UPDATE` で非同期の重複INSERTに対して冪等
- `direction` は監視ウォレット視点のため `user_transactions_t` 側に配置
